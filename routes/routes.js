...
router.post("/producer/submit", async (req, res) => {
	...
	if (currentDay && currentDay[slot] && currentDay[slot].dj) {
		...
		const djName =
			req.body.djs.charAt(0).toUpperCase() + req.body.djs.slice(1);
...
router.post("/dj", async (req, res) => {
	...
	try {
		...
		const isSongExists = playlist.songs.some(
			(song) => song.title === songTitle && song.artist === artist
		);
		if (!isSongExists) {
